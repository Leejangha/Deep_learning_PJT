...
if __name__ == '__main__': # 현재 파일이 메인 프로그램일 경우 작동
    ...
    parser.add_argument('--num_epochs', type=int, default=100) # EfficientNet 모델 학습을 위한 Epoch 입력
    parser.add_argument('--num_epochs_decay', type=int, default=70) # EfficientNet 모델의 Learning Rate 설정을 위한 num_epochs_decay 입력
    ...
    parser.add_argument('--num_workers', type=int, default=20) # 데이터 로더 설정을 위한 num_workers 입력
    parser.add_argument('--lr', type=float, default=0.0001) # EfficientNet 모델 학습에 사용할 Learning Rate 입력
    parser.add_argument('--beta1', type=float, default=0.5) # EfficientNet 모델 학습에 사용할 beta1 입력
    parser.add_argument('--beta2', type=float, default=0.999) # EfficientNet 모델 학습에 사용할 beta2 입력 
    parser.add_argument('--patience', type=int, default=7) # EfficientNet 모델 학습에 사용할 Early Stop Patience 입력
    parser.add_argument('--model_type', type=str, default='Efficientnet', help='Resnet/Efficientnet') # 모델 유형 입력(Resnet/Efficientnet)
    parser.add_argument('--model_path', type=str, default='./code_with_dataset/PRPD_Classification/EfficientNet_Result') # 학습 결과 저장 경로 입력
    ...
    parser.add_argument('--result_path', type=str, default='./code_with_dataset/PRPD_Classification/EfficientNet_Result/') # 모델 학습 결과 저장 경로 입력
